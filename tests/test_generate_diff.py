...
def test_generate_diff():
    ...
    assert diff_json2_plain == open("tests/fixtures/correct_result_tree_plain.txt").read().strip()
    
    diff_yaml2_plain = generate_diff("tests/fixtures/file1_tree.yaml", "tests/fixtures/file2_tree.yaml", format_name='plain')
    assert diff_yaml2_plain == open("tests/fixtures/correct_result_tree_plain.txt").read().strip()
    
    diff_json3_json = generate_diff("tests/fixtures/file1_tree.json", "tests/fixtures/file2_tree.json", format_name='json')
    assert diff_json3_json == open("tests/fixtures/correct_result_tree_json.txt").read().strip()
    
    diff_yaml3_json = generate_diff("tests/fixtures/file1_tree.yaml", "tests/fixtures/file2_tree.yaml", format_name='json')
    assert diff_yaml3_json == open("tests/fixtures/correct_result_tree_json.txt").read().strip()
    
    diff_json_to_json = generate_diff("tests/fixtures/file1.json", "tests/fixtures/file2.json", format_name='json')
    assert diff_json_to_json == open("tests/fixtures/correct_result_json_format.txt").read().strip()
